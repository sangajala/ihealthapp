...
namespace Sree_Health.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    model.UserRole = GetUserRole(model.Email);
                    return RedirectToLocal(returnUrl, model.UserRole);
        ...
        public async Task<ActionResult> VerifyCode(VerifyCodeViewModel model)
        {
            ...
            {
                case SignInStatus.Success:
                    return RedirectToLocal(model.ReturnUrl, 0);
                case SignInStatus.LockedOut:
                    return View("Lockout");
                case SignInStatus.Failure:
                default:
        ...
        [HttpPost]
        public JsonResult ValidateEmail(string Email, string duplicate_code)
        {
            if (Email == duplicate_code)
                return Json(true);
            BusinessLib blib = new BusinessLib();
            PropertyLib plib = new PropertyLib();
            plib.Email = Email;
            bool exists = blib.ValidateEmail(plib);
            if (exists == true)
                return Json(false);
            return Json(true);
        }
        ...
        [AllowAnonymous]
        public ActionResult Register(string Email, int UserRole)
        {
        ...
        // GET: /Account/Register
        [AllowAnonymous]
        public ActionResult Register_All()
        {
            RegisterViewModel model = new RegisterViewModel();
            //model.UsersList =  LoadUsersList();
            return View(model);
        }
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Register_All(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email, UserRole = model.UserRole };
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Register_Patient(RegisterPatientViewModel model)
        {
            if (ModelState.IsValid)
            {
                try
                ...
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email, UserRole = 3 };
                var result = await UserManager.CreateAsync(user, model.Password);
                if (result.Succeeded)
                {
        ...
        private ActionResult RedirectToLocal(string returnUrl, int UserRole)
        {
        ...
        public int GetUserRole(string Email)
        {
            ...
            PropertyLib plib = new PropertyLib();
            plib.Email = Email;
            return blib.GetUserRole(plib);
        }
        //public void RegisterDoctor(RegisterViewModel model)
        //{
        //    plib.UserID = User.Identity.GetUserId() ?? "";

        //}
        public List<UsersListModel> LoadUsersList()
        {
            BusinessLib blib = new BusinessLib();
            DataTable dataTable = blib.Load_UserEmails();
        ...
        private void InsertPatients(RegisterPatientViewModel model)
        {
            BusinessLib blib = new BusinessLib();
            PropertyLib plib = new PropertyLib();
